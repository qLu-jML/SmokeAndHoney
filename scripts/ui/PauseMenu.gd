...
func _build_ui() -> void:
	...
	var btn_data: Array = [
		...
		["How am I feeling?", Callable(self, "_on_feeling")],
...
## Winter Workshop S3: Self-assessment -- qualitative fatigue description.
func _on_feeling() -> void:
	var pct: float = GameData.energy / GameData.max_energy
	var desc: String
	if pct >= 0.70:
		desc = "You feel rested and ready to work. Full day ahead."
	elif pct >= 0.50:
		desc = "A little tired. You could use a break soon, but you can keep going."
	elif pct >= 0.25:
		desc = "Pretty worn out. Your body is telling you to slow down."
	elif pct >= 0.10:
		desc = "Exhausted. Everything feels heavy. You should eat or rest."
	else:
		desc = "You can barely keep your eyes open. Time to call it a day."
