...
# GDD S3.0a: Name, backstory selection before first play.
...
const PANEL_Y   := 25     # vertically centered
const FONT_SM   := 7
const FONT_MD   := 8
const FONT_LG   := 9
...
# -- Focus section (0=name, 1=backstory, 2=confirm) ---------------------------
var _focus: int = 0
const FOCUS_COUNT := 3
...
var _section_bgs: Array = []   # 3 ColorRects (name, backstory, confirm)
...
func _handle_key(event: InputEventKey) -> void:
    ...
    if event.keycode == KEY_TAB or event.keycode == KEY_S:
        if not (event.shift_pressed):
            _focus = (_focus + 1) % FOCUS_COUNT
            _refresh()
    if event.keycode == KEY_W:
        _focus = (_focus - 1 + FOCUS_COUNT) % FOCUS_COUNT
        _refresh()
...
func _handle_choice_input(event: InputEventKey) -> void:
...
func _try_confirm() -> void:
    ...
    # Default pronouns to they/them
    if PlayerData.has_method("set_pronouns_they_them"):
        PlayerData.set_pronouns_they_them()
...
func _refresh_sections() -> void:
    ...
    if is_instance_valid(_confirm_lbl):
        _confirm_lbl.modulate = Color(0.4, 0.9, 0.4, 1.0) if _focus == 2 else Color(0.8, 0.75, 0.55, 1.0)
...
func _build_ui() -> void:
    ...
    var section_y: Array = [PANEL_Y + 22, PANEL_Y + 50, PANEL_Y + 98]
    var section_h: Array = [26, 44, 18]
    ...
    _backstory_lbl.position            = Vector2(PANEL_X + 8, PANEL_Y + 52)
    ...
    _desc_lbl.position            = Vector2(PANEL_X + 8, PANEL_Y + 64)
    ...
    div2.position = Vector2(PANEL_X + 4, PANEL_Y + 96)
    ...
    _confirm_lbl.position              = Vector2(PANEL_X, PANEL_Y + 100)
    ...
    hint.position              = Vector2(PANEL_X, PANEL_Y + 115)
    ...
    _err_lbl.position              = Vector2(PANEL_X, PANEL_Y + 123)
