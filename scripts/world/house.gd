...
const W: int = 11
const H: int = 11
const DOOR_COL: int = 5   # 0-indexed centre column

# Colors
const COL_WALL: Color = Color(0.62, 0.62, 0.70, 1.0)
const COL_DOOR: Color = Color(0.55, 0.27, 0.07, 1.0)
const COL_EDGE: Color = Color(0.28, 0.28, 0.32, 1.0)

var _transitioning: bool = false

## Initialize the house: draw in editor mode, build collision in runtime.
func _ready() -> void:
...
## Draw the house walls and door using filled rectangles.
func _draw() -> void:
	for row in range(H):
		for col in range(W):
			var rect: Rect2 = Rect2(Vector2(col, row) * TILE, Vector2(TILE, TILE))
...
## Build collision shapes for the house walls.
...
## Add a collision shape to the given StaticBody2D.
...
## Trigger house entry transition.
...
## Save exterior hives and flowers state for restoration on re-entry.
func _save_state() -> void:
	TimeManager.exterior_hives.clear()
	for h in get_tree().get_nodes_in_group("hive"):
		var entry: Dictionary = {"pos": Vector2(h.global_position)}
